...
export function POPDFPreview({ poId, poNumber }: POPDFPreviewProps) {
  ...
  useEffect(() => {
    ...
    setPdfUrl(`/api/finance/purchase-orders/${poId}/preview-pdf?t=${Date.now()}`)
  }, [poId, refreshKey])
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      const response = await fetch(`/api/finance/purchase-orders/${poId}/excel`)
      if (!response.ok) {
