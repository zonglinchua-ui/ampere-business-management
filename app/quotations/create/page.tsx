...
function CreateQuotationForm() {
  ...
  // Check if this is a variation order
  const isVariationOrder = searchParams?.get('isVariationOrder') === 'true'
  ...
  const getStandardTerms = () => {
    return `1. Quotation is valid for 30 days from the date of issue.
...
4. Any variations to the original scope of work will be changed separately.
...
This document is computer generated. No signature is required.`
  ...
  const [quotationData, setQuotationData] = useState({
    customerId: "",
    tenderId: "no-tender",
    title: isVariationOrder ? "Variation Order" : "",
    description: "",
    clientReference: "",
    salespersonId: session?.user?.id || "",
    validUntil: "", // Initialize empty to avoid hydration mismatch
    validityDays: 30, // Default 30 days validity
    currency: "SGD",
    terms: "", // Will be populated with standard terms on client side
    notes: "",
    templateType: "standard",
    additionalTerms: "",
    // Payment terms structure
    paymentTerms: [
      { percentage: 50, description: "Upon order confirmation" },
      { percentage: 50, description: "Upon completion" }
    ],
    // Quotation-level discount and tax
    discountPercentage: 0,
    taxPercentage: 9, // Default 9% GST
    projectId: projectId,
    isVariationOrder: isVariationOrder,
    variationOrderType: isVariationOrder ? 'ADDITION' : null
  })
  ...
  useEffect(() => {
    // Fetch project details if creating variation order from project
    if (isVariationOrder && projectId) {
      fetchProjectDetails()
    }
  }, [isVariationOrder, projectId])

  useEffect(() => {
    // Set default validity date and standard terms on client side to avoid hydration mismatch
    if (!quotationData.validUntil || !quotationData.terms) {
      setQuotationData(prev => ({
        ...prev,
        validUntil: prev.validUntil || getDefaultValidityDate(),
        terms: prev.terms || getStandardTerms()
      }))
    }
  }, [])
  ...
  const fetchProjectDetails = async () => {
    try {
      ...
      // Fetch existing variation orders for this project to determine the next number
      const variationOrdersResponse = await fetch(`/api/quotations?projectId=${projectId}&isVariationOrder=true`)
      const variationOrdersData = await variationOrdersResponse.json()
      const existingVariationOrders = variationOrdersData.data || []
      const nextNumber = existingVariationOrders.length + 1
      
      // Auto-fill customer and generate title
      setQuotationData(prev => ({
        ...prev,
        customerId: project.customerId,
        title: `${project.name} Variation Order ${nextNumber}`
      }))
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(message, {
          description: `${quotationData.quotationNumber} has been created and is now in the quotations list.`,
          duration: 4000
        })
        
        // Navigate to quotations list
        // This avoids the timing issue where detail page loads before DB commit
  ...
  const canProceedToNext = () => {
    switch (currentStep) {
      case 1:
        return quotationData.customerId && quotationData.title && quotationData.validUntil
      case 2:
      ...
      case 3:
        return true
      case 4:
        // Final step - allow creation if basic validation passes
        return quotationData.customerId && quotationData.title && quotationData.validUntil
      default:
        return false
    }
  ...
  const canSaveDraft = () => {
    ...
    return quotationData.customerId && quotationData.title
  }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  ...
                  <DynamicSelect
                    ...
                    onValueChange={(customerId) => setQuotationData({...quotationData, customerId})}
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setQuotationData({...quotationData, terms: getStandardTerms()})}
                  className="text-xs"
                >
              ...
              <CardDescription>
                Configure payment terms, additional clauses, and standard T&Cs
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {/* Info Note */}
              ...
              <div className="border rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      const newPaymentTerms = [...(quotationData.paymentTerms as any[])]
                      newPaymentTerms.push({ percentage: 0, description: "" })
                      setQuotationData({...quotationData, paymentTerms: newPaymentTerms})
                    }}
                  >
                    <Plus className="h-3 w-3 mr-1" />
                    Add Payment Term
                ...
                <div className="space-y-3">
                  {(quotationData.paymentTerms as any[]).map((term: any, index: number) => (
                        ...
                        <div className="flex gap-2">
                          <div className="w-24">
                            <Input
                              ...
                              onChange={(e) => {
                                const newPaymentTerms = [...(quotationData.paymentTerms as any[])]
                                newPaymentTerms[index].percentage = parseInt(e.target.value) || 0
                                setQuotationData({...quotationData, paymentTerms: newPaymentTerms})
                              }}
                              className="text-sm"
                            />
                          ...
                          <div className="flex-1">
                            <Input
                              placeholder="e.g., Upon order confirmation"
                              value={term.description}
                              onChange={(e) => {
                                const newPaymentTerms = [...(quotationData.paymentTerms as any[])]
                                newPaymentTerms[index].description = e.target.value
                                setQuotationData({...quotationData, paymentTerms: newPaymentTerms})
                              }}
                              className="text-sm"
                            />
                ...
                {/* Calculate and show total percentage */}
                <div className="mt-3 pt-3 border-t">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Total Percentage:</span>
                    <span className={`font-medium ${
                      (quotationData.paymentTerms as any[]).reduce((sum: number, term: any) => sum + (term.percentage || 0), 0) === 100
                        ? 'text-green-600'
                        : 'text-red-600'
                    }`}>
                      {(quotationData.paymentTerms as any[]).reduce((sum: number, term: any) => sum + (term.percentage || 0), 0)}%
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <Textarea 
                  placeholder="Add any additional terms and conditions specific to this quotation (e.g., special clauses, exclusions, etc.)"
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  These will be included in the Terms & Conditions section of the PDF
                </p>
              </div>
              ...
              <div>
                ...
                <Textarea 
                  placeholder="Loading standard terms and conditions..."
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Standard terms and conditions are pre-loaded. Modify as needed for this specific quotation.
                </p>
              </div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {quotationData.validUntil ? (() => {
                          try {
                            return format(new Date(quotationData.validUntil), 'MMM dd, yyyy')
                          } catch (error) {
                            return quotationData.validUntil
                          }
                        })() : 'Not set'}
                      </span>
                    </div>
