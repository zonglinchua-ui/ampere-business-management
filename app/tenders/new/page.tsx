...
interface TenderFormData {
  ...
  contactPhone?: string
  location?: string
  category: 'CONSTRUCTION' | 'ENGINEERING' | 'SUPPLY' | 'CONSULTING' | 'MAINTENANCE' | 'INSTALLATION' | 'REINSTATEMENT' | 'GENERAL'
  assignedToId?: string
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          toast.error(responseData.error || 'You must be logged in to create tenders')
        ...
        if (response.status === 400) {
          ...
          if (responseData.validationErrors) {
            const errorMessages = Object.entries(responseData.validationErrors)
            ...
            toast.error(
              <div className="space-y-1">
                ...
                {Object.entries(responseData.validationErrors).map(([field, message]) => (
                  <div key={field} className="text-sm">• {message as string}</div>
                ))}
              </div>,
              { duration: 6000 }
            )
          } else {
            toast.error(responseData.error || 'Invalid input. Please check your form.')
          }
          return
        }

        if (response.status === 403) {
          toast.error('You do not have permission to create tenders')
          return
        }

        if (response.status === 409) {
          toast.error(responseData.error || 'A tender with this information already exists')
          return
        }

        // For any other non-OK response
        toast.error(responseData.error || 'Failed to create tender. Please try again.')
        return
      }

      // Success case
      console.log('Tender created successfully:', responseData)
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select onValueChange={(value) => {
                      // Only set valid customer IDs, ignore disabled options
                      if (value !== "no-customers") {
