...
import { RefreshCw, Plus, Download, AlertTriangle, FileText, Link2, Database, ChevronDown, ShoppingCart, Building2, Eye, CheckCircle, Clock, Truck, XCircle, ArrowRight } from "lucide-react"
...
import { XeroSyncStatus } from "@/components/finance/xero-sync-status"
import { useXeroStatus } from "@/hooks/use-xero-status"

// Lazy load heavy chart components for better initial performance
const MonthlyRevenueExpensesChart = lazy(() => import("@/components/finance/monthly-revenue-expenses-chart").then(mod => ({ default: mod.MonthlyRevenueExpensesChart })))
const YearlyRevenueExpensesChart = lazy(() => import("@/components/finance/yearly-revenue-expenses-chart").then(mod => ({ default: mod.YearlyRevenueExpensesChart })))
...
export default function FinancePage() {
  ...
  // Use optimized Xero status hook
  const { isConnected: isXeroConnected, lastSyncTime, isLoading: checkingXeroStatus, refresh: refreshXeroStatus } = useXeroStatus()
  ...
  // Fetch financial data on mount and period change (non-blocking)
  useEffect(() => {
    if (canAccessFinance) {
      // Run fetch in background without blocking UI
      fetchFinancialData().catch(err => {
        console.error('[Finance] Background data fetch error:', err)
  ...
  // Refresh function for Xero status (used by callback handler)
  ...
  const fetchFinancialData = async () => {
    try {
      // Don't block UI with loading state - data loads in background
  ...
  const fetchProjects = async () => {
    try {
      const res = await fetch('/api/projects?limit=1000') // Fetch all projects for dropdown
      if (res.ok) {
        const data = await res.json()
        // Handle both paginated and direct array responses
        const projectsArray = data.projects || data.data || data
        console.log('[Finance] Fetched projects:', projectsArray?.length || 0)
  ...
  const fetchBudgetCategories = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        // API returns { systemCategories, customCategories }
        // Merge both into a single array for the dropdown
        const allCategories = [
          ...(data.systemCategories || []).map((cat: any) => ({
            id: cat.value,
            name: cat.label,
            code: cat.value,
            color: '#6b7280' // Default gray color for system categories
          })),
          ...(data.customCategories || [])
        ]
        console.log('[Finance] Fetched budget categories:', allCategories.length)
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              {/* Xero Sync Status - Always visible */}
              <XeroSyncStatus
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      <h4 className="text-sm font-semibold text-blue-900 dark:text-blue-100">Unsynced Project Invoices</h4>
                      <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                        View and manage project-related invoices (customer & supplier) that are automatically tagged and awaiting sync to Xero.
                      </p>
                    </div>
                  </div>
                  <Link href="/finance/unsynced-invoices">
                    <Button size="sm" variant="outline" className="border-blue-300 hover:bg-blue-100 dark:border-blue-700 dark:hover:bg-blue-900">
                      View Unsynced Invoices
